...
.PHONY: test
test: $(NODE_MODULES) $(JEST) ## Run unit tests.
	@$(JEST) $(TEST_FILES) --verbose


.PHONY: coverage
coverage: $(NODE_MODULES) $(JEST) ## Run unit tests with coverage reporting. Generates reports into /coverage.
	@$(JEST) $(TEST_FILES) --coverage
...
report-coverage: $(NODE_MODULES) $(NYC) ## Run unit tests with coverage reporting. Generates reports into /coverage.
	@$(JEST) $(TEST_FILES) --coverage --coverageReporters=text-lcov | $(COVERALLS)
